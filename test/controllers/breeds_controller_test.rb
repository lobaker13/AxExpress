...
class BreedsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create breed" do
    assert_difference('Breed.count') do
      post breeds_url, params: { breed: { name: @breed.name, species: @breed.species, status: @breed.status } }
    end
  ...
  test "should update breed" do
    patch breed_url(@breed), params: { breed: { name: @breed.name, species: @breed.species, status: @breed.status } }
